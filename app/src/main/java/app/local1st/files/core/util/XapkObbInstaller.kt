...
object XapkObbInstaller {
    ...
    class Placement internal constructor(
        ...
        fun cleanUp() {
            writtenFiles.forEach { file ->
                if (!file.delete() && RootAccess.usable()) {
                    runCatching { RootShell.exec("rm -f -- ${RootShell.quote(file.path)}") }
                }
            }
            backups.forEach { (destination, backup) ->
                ...
                if (!restored && XapkObbInstaller.rootWritable()) {
                    runCatching {
                        RootShell.exec(
                            "mv -f -- ${RootShell.quote(backup.path)} ${RootShell.quote(destination.path)}",
                        )
                    }
                }
            }
        ...
        fun commit() {
            backups.forEach { (_, backup) ->
                if (!backup.delete() && XapkObbInstaller.rootWritable()) {
                    runCatching { RootShell.exec("rm -f -- ${RootShell.quote(backup.path)}") }
                }
            }
        }
    }
    ...
    private fun backUp(destination: File): File {
        ...
        try {
            RootShell.exec(
                "mv -f -- ${RootShell.quote(destination.path)} ${RootShell.quote(backup.path)}",
            )
            return backup
        } catch (e: Exception) {
    ...
    private fun destinationSize(file: File): Long {
        ...
        return RootShell.exec("stat -c %s -- ${RootShell.quote(file.path)} 2>/dev/null || echo -1")
    ...
    private fun writeWithRoot(input: java.io.InputStream, destination: File): Boolean {
        val parent = destination.parentFile ?: throw IOException("Invalid OBB destination")
        val temp = File(parent, ".${destination.name}.xfiles-${UUID.randomUUID()}.part")
        val quotedParent = RootShell.quote(parent.path)
        val quotedTemp = RootShell.quote(temp.path)
        val quotedDestination = RootShell.quote(destination.path)
        RootShell.exec("mkdir -p -- $quotedParent && chmod 0755 $quotedParent")
        try {
            RootShell.openWrite(temp.path).use { output -> input.copyTo(output) }
            ...
            runCatching { RootShell.exec("rm -f -- $quotedTemp") }
            throw e
        }
    }

    private fun rootWritable(): Boolean =
        RootAccess.enabled && !RootAccess.readOnly && RootShell.isAvailable()
